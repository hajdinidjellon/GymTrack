...
| B5 | 🔶 **PARTIEL 2026-07-05** — RLS Supabase | Script complet prêt : **`supabase/setup.sql`** (tables idempotentes, RLS + policies sur les 4 tables, fonction `delete_user()` pour B7). **Reste (2 min)** : le coller dans le SQL Editor du dashboard Supabase et exécuter, puis test anon key de [05-SECURITY.md](05-SECURITY.md) §3. | S |
| B6 | **Tokens auth → SecureStore** | Session Supabase actuellement dans AsyncStorage en clair. | M |
| B7 | ✅ **FAIT 2026-07-05** — Suppression de compte + export données | Profil → Réglages : « Exporter mes données » (JSON complet via partage système, `lib/account.ts`) et « Supprimer mon compte » (cloud d'abord — échec cloud = local intact —, puis RPC `delete_user`, puis wipe local + AsyncStorage). Dépend de l'exécution de `supabase/setup.sql` (B5) pour la partie auth. | M |
| B8 | 🔶 **PARTIEL 2026-07-05** — Politique de confidentialité | Rédigée : **`docs/legal/privacy-policy.html`** (fr + résumé en, spécifique aux données réelles de l'app, droits intégrés in-app référencés). **Reste** : héberger (GitHub Pages/Netlify), vérifier la région UE du projet Supabase (§5), coller l'URL dans les fiches stores. | M |
...
| B12 | ✅ **FAIT 2026-07-05** — `eas.json` + projet EAS + env vars | `eas.json` (profils dev/preview/production) ; projet lié : `@2late/gym-tracker-mobile` (projectId `e3bf537c…` dans `app.json`) ; `EXPO_PUBLIC_SUPABASE_URL`/`ANON_KEY` créées dans les 3 environnements EAS. **Prochain pas quand tu veux** : `eas build --profile preview --platform android` (après B9 idéalement, pour ne pas builder 56 MB d'assets). | M |
